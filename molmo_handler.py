...
# Configuración de logging para mostrar la línea de código donde ocurre el error
...
class EndpointHandler:
    ...
    def __call__(self, data=None):
        ...
        # Obtener prompts globales y específicos y agruparlos
        try:
            ...
            for idx, img_id in enumerate(ids):
                try:
                    ...
                    logging.exception("Error procesando prompt para image id %s", img_id)
            # Fin de agrupación de prompts
        except Exception:
            logging.exception("Error al construir los grupos de prompts")
            return {"error": "Error al agrupar los prompts."}

        # Configuración de generación y procesamiento de cada grupo
        try:
            ...
            for prompt_text, group in prompt_groups.items():
                ...
                for start in range(0, len(group_images), batch_size):
                    try:
                        ...
                        logging.exception("Error procesando el batch que inicia en el índice %d para prompt '%s'", start, prompt_text)
            # Fin de procesamiento de grupos
        except Exception:
            logging.exception("Error al procesar los grupos de prompts")
            return {"error": "Error al procesar los grupos de prompts."}
